...
open class RPCircularProgress: UIView {
    ...
    lazy fileprivate var thinIndeterminate: RPCircularProgress = {
        ...
        progress.progressTintColor = UIColor.init(named: "LightBlue") ?? UIColor.blue
    ...
    public func show() {
        ...
        if let topController = currentViewController {
            
            let superView: UIView = topController.view
            //superView.addSubview(self)
            container.backgroundColor = UIColor.init(named: "GreyTransparent")
            superView.addSubview(container)
            container.snp.makeConstraints { (make) in
                make.center.equalTo(superView)
                make.width.equalTo(superView).multipliedBy(1)
                make.height.equalTo(superView).multipliedBy(1)
            }
            
            
            container.addSubview(thinIndeterminate)
            thinIndeterminate.snp.makeConstraints { (make) in
    ...
    public func initialize(view: UIView) {
